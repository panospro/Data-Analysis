...
%     title('Sample SD of psoitivity rate i days before')
    ...
    %%  PCA and estimate of d<p using different techniques
    % center data at 0
    X = X - repmat(sum(X)/n,n,1);
    % We find the table with dispersions
    covxM = cov(X);
...
%     xlabel('index')
%     ylabel('eigenvalue')
%     legend('how the eigenvalues change, from biggest to highest','the mean')
